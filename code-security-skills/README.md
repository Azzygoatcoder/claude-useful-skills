...
> Claude Code 插件 — 系统化代码安全审计技能集，覆盖漏洞发现、验证、报告、修复、重审计全流程。**v1.3.0**
...
```bash
claude plugins install https://github.com/Azzygoatcoder/claude-useful-skills --path code-security-skills
```

Claude Code 会自动克隆仓库并注册 `code-security-skills/skills/` 目录下的全部技能。安装后重启 Claude Code 会话即可生效。
...
| **1.1.1** | 2026-06-18 | 自审计修复：XSS 单引号转义、git add -u、示例占位符化、plugin.json 元数据、路径校验、atexit 文档 |
